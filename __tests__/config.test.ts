...
test('readVotingConfig has no error on missing percentageToApprove', async () => {
  ...
  ).resolves.toEqual(new VotingConfig({percentageToApprove: 0, minVotersRequired: 10, minVotingWindowMinutes: 3600}));
})
...
test('readVotingConfig has error on invalid minVotersRequired', async () => {
  ...
  ).resolves.toEqual(new VotingConfig({percentageToApprove: 75, minVotersRequired: 0, minVotingWindowMinutes: 3600}));
})

test('readVotingConfig has error on invalid minVotingWindowMinutes', async () => {
  await expect(
    readVotingConfig('./__tests__/data/voting-invalid-minVotingWindowMinutes.yml')
  ).resolves.toEqual(new VotingConfig({percentageToApprove: 75, minVotersRequired: 10, minVotingWindowMinutes: 0}));
})
